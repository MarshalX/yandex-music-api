...
class TestAlbum:
    ...
    def test_expected_values(self, album, artist, label, track_position, track_without_albums):
    ...
    def test_de_json_required(self, client, artist_without_tracks, label):
        json_dict = {'id': self.id, 'title': self.title, 'cover_uri': self.cover_uri, 'track_count': self.track_count,
                     'artists': [artist_without_tracks.to_dict()], 'labels': [label.to_dict()],
    ...
    def test_de_json_all(self, client, artist_without_tracks, label, track_position, track_without_albums):
        json_dict = {'id': self.id, 'title': self.title, 'cover_uri': self.cover_uri, 'track_count': self.track_count,
                     'artists': [artist_without_tracks.to_dict()], 'labels': [label.to_dict()],
                     ...
                     'volumes': [[track_without_albums.to_dict()]], 'year': self.year,
    ...
    def test_equality(self, artist_without_tracks, label):
        a = Album(self.id, self.title, self.track_count, [artist_without_tracks], [label],
                  self.available, self.available_for_premium_users)
        b = Album(10, '', 99, [artist_without_tracks], [label],
                  self.available, self.available_for_premium_users)
        c = Album(self.id, self.title, self.track_count, [artist_without_tracks], [label],
                  self.available, self.available_for_premium_users)
