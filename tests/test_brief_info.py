...
def brief_info(artist, track, album, playlist, cover, playlist_id, video, chart, vinyl):
    return BriefInfo(artist, [album], [playlist], [album], TestBriefInfo.last_release_ids, [album], [track], [artist],
                     [cover], TestBriefInfo.concerts, [video], [vinyl], TestBriefInfo.has_promotions, [playlist_id],
                     [chart])
...
class TestBriefInfo:
    ...
    def test_de_json_required(self, client, artist, track, album, playlist, cover, playlist_id, video, vinyl):
        json_dict = {'artist': artist.to_dict(), 'albums': [album.to_dict()], 'also_albums': [album.to_dict()],
                     'last_release_ids': self.last_release_ids, 'last_releases': [album.to_dict()],
                     'popular_tracks': [track.to_dict()], 'similar_artists': [artist.to_dict()],
                     'all_covers': [cover.to_dict()], 'concerts': self.concerts, 'videos': [video.to_dict()],
                     'vinyls': [vinyl.to_dict()], 'has_promotions': self.has_promotions,
    ...
    def test_equality(self, artist, track, album, playlist, cover, playlist_id, video, vinyl):
        a = BriefInfo(artist, [album], [playlist], [album], self.last_release_ids, [album], [track], [artist], [cover],
        ...
        c = BriefInfo(artist, [album], [playlist], [album], [1, 2, 3], [album], [track], [artist], [], self.concerts,
                      [video], [vinyl], self.has_promotions, [playlist_id])
        d = BriefInfo(artist, [album], [playlist], [album], self.last_release_ids, [album], [track], [artist], [cover],
                      self.concerts, [video], [vinyl], self.has_promotions, [playlist_id])
