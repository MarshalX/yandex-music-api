...
Эта библиотека предоставляет неофициальный Python интерфейс для недокументированного API Яндекс Музыки.

Она совместима с версиями Python 3.7+.

Данная библиотека предоставляет ряд различных оберток высокого уровня, чтобы сделать разработку более простой и понятной.
...
взяты из официального приложения сервиса Яндекс.Музыка на UWP-платформу. Поскольку API является закрытым и используется только внутри
компании Яндекс сейчас невозможно зарегистрировать своё собственное приложение на
`oauth.yandex.ru <https://oauth.yandex.ru/>`_, из чего следует, то вы не можете определите свои значения этих констант.
...
Вы можете установить или обновить yandex-music-api при помощи pip:
...
Для начала работы вы должны инициализировать клиент:
...
Для доступа к личным данным необходимо авторизоваться.
...
Создав экземпляр клиента, у вас появляется возможность вызвать необходимый метод Client из API. Подробнее о методах клиента
