...
Радио (внутреннее название rotor) представляет собой бесконечный поток треков, который подбирается под пользователя и уточняется по ходу прослушивания. Сюда относятся «Моя волна», волны по занятиям и настроению, потоки по жанру, исполнителю, альбому или треку, а также генеративные станции с музыкой от нейросети.
...
Сиды можно комбинировать: `['user:onyourwave', 'settingDiversity:discover']` даёт «Моя волна» только с незнакомыми треками.
...
Сессия является актуальным способом слушать радио, им пользуются приложения Яндекс Музыки. Сессия создаётся по сидам, выдаёт треки партиями и принимает обратную связь о прослушивании, по которой подбирает следующие треки.
...
|---------|--------------------------------------|
| `radioStarted`, `combinedQueueStarted`, `ad` | нет |
