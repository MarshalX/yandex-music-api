...
if TYPE_CHECKING:
    from yandex_music import ClientType, GenerativeStreamData, GenerativeStreamInfo, JSONType
...
class GenerativeStream(YandexMusicModel):
    ...
    @classmethod
    def de_json(cls, data: 'JSONType', client: 'ClientType') -> Optional['GenerativeStream']:
        ...
        Returns:
            :obj:`yandex_music.GenerativeStream`: Информация о генеративной станции.
        """
        if not cls.is_dict_model_data(data):
            return None

        cls_data = cls.cleanup_data(data, client)
        from yandex_music import GenerativeStreamData, GenerativeStreamInfo

        cls_data['data'] = GenerativeStreamData.de_json(cls_data.get('data'), client)
        cls_data['stream'] = GenerativeStreamInfo.de_json(cls_data.get('stream'), client)
