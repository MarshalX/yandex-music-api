...
- [ynison_remote](ynison_remote.py) – интерактивный удалённый пульт Ynison.
  Выводит текущее состояние плеера и позволяет управлять активным устройством
  (пауза/продолжить, переключение треков, громкость). **Модуль Ynison в бете:**
  публичное API может меняться без соблюдения semver.
- [wave_radio](wave_radio.py) – консольное радио «Моя волна» на сессиях радио.
  Выбор волны (Моя волна или занятие) и её характера, воспроизведение через
  установленный в системе плеер, пропуск трека, «Нравится» и «Не нравится».
  Обратная связь о прослушиваниях отправляется в сессию, чтобы радио подбирало
  следующие треки с её учётом. Без сторонних Python-зависимостей.
